################################################################################
# SALMON ANALYSIS - YUKON REGIONAL ANALYSIS
# Analysis 1: Yuk_US (Upper Yukon only)
# Analysis 2: Yuk_Canada (Lower & Middle Yukon combined)
################################################################################
...
PATHS <- list(
  ...
  # ── Outputs ───────────────────────────────────────────────
  output_yuk_us = "C:/Users/makhl/Research Repos/Shifting-Habitat-Mosaics-II/Outputs/ProductionData/Yuk_US",
  output_yuk_canada = "C:/Users/makhl/Research Repos/Shifting-Habitat-Mosaics-II/Outputs/ProductionData/Yuk_Canada"
)

MAP_OUTPUT_DIR_US <- "C:/Users/makhl/Research Repos/Shifting-Habitat-Mosaics-II/Figures/Maps/Yuk_US"
MAP_OUTPUT_DIR_CANADA <- "C:/Users/makhl/Research Repos/Shifting-Habitat-Mosaics-II/Figures/Maps/Yuk_Canada"

# Analysis parameters
min_stream_order <- 5
min_error <- 0.0035
sensitivity_threshold <- 0.7
years_to_process <- c(2015, 2016, 2018, 2021)
...
cat("\n################################################################################\n")
cat("# ANALYSIS 1: YUK_US (UPPER YUKON ONLY)\n")
cat("################################################################################\n\n")

for (year in years_to_process) {
  
  cat(paste("\n=== Processing Yuk_US (Upper only)", year, "===\n"))
  ...
  tryCatch({
    ...
    colcode[basin_assign_norm <=.7] <- "white"
    
    colcode[basin_assign_norm > 0.7 & basin_assign_norm <= 0.75] <- palette[4]
    colcode[basin_assign_norm > 0.75 & basin_assign_norm <= 0.8] <- palette[5]
    colcode[basin_assign_norm > 0.8 & basin_assign_norm <= 0.85] <- palette[6]
    colcode[basin_assign_norm > 0.85 & basin_assign_norm <= 0.9] <- palette[7]
    colcode[basin_assign_norm > 0.9 & basin_assign_norm <= 0.95] <- palette[8]
    colcode[basin_assign_norm > 0.95 & basin_assign_norm <= 1.0] <- palette[9]
    ...
    plot(st_geometry(basin), col = "gray60", border = "gray60", 
         main = paste0("Annual Production - Upper Yukon (Yuk_US)\nYear: ", year), 
         bg = "white")
...
cat("\n################################################################################\n")
cat("# ANALYSIS 2: YUK_CANADA (LOWER & MIDDLE YUKON COMBINED)\n")
cat("################################################################################\n\n")

for (year in years_to_process) {
  
  cat(paste("\n=== Processing Yuk_Canada (Lower + Middle)", year, "===\n"))
  ...
  tryCatch({
    ...
    plot(st_geometry(basin), col = "gray60", border = "gray60", 
         main = paste0("Annual Production - Lower & Middle Yukon (Yuk_Canada)\nYear: ", year), 
         bg = "white")
    plot(st_geometry(edges), col = colcode, pch = 16, axes = FALSE, 
         add = TRUE, lwd = linewidths)
    legend("topleft", legend = legend_labels, col = legend_colors, lwd = 5,
