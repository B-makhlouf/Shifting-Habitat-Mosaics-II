...
plots_col1 <- lapply(seq_along(years), function(i) {
  ...
  p <- ggplot(df, aes(x = mean_summer_temp, y = Channel_sl)) +
    annotate(
      ...
      xmin = -Inf, xmax = Inf,
      ymin = -Inf, ymax = Inf,
      fill = "white"  # Changed from brewer.pal(9, "YlOrRd")[1]
    ) +
    ...
    labs(
      x = if(i == 5) "Mean Summer Temperature (°C)" else NULL,
      y = "Channel Slope"
    ) +
    theme_minimal() +
    theme(
      axis.text = element_text(size = 9),
      axis.title.x = if(i == 5) element_text(size = 10) else element_blank(),
...
plots_col2 <- lapply(seq_along(years), function(i) {
  ...
  ggplot(df, aes(x = SNAP_temp, y = Channel_sl)) +
    ...
    scale_y_continuous(limits = c(0, 3), expand = c(0, 0)) +
    labs(
      x = if(i == 5) "SNAP Air Temperature (°C)" else NULL,
      y = NULL
    ) +
    theme_minimal() +
    theme(
