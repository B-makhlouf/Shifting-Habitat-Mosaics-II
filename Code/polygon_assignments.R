...
all_ind_data<- read_csv(here("2015 Yukon .99_tributaries.csv"))

tribs<- yuk_polygons$Trib

#list out all unique values of the "Individual" column

length(unique(all_ind_data$Individual))
...
for (i in 1:length(unique(all_ind_data$Individual))){
  ...
  for (b in 1:length(tribs)){
    if (tribs[b] %in% ind_data$tribs){
      ind_data_vector[b]<- 1
    }
  }
