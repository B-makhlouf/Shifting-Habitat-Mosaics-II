################################################################################
# 01_VISUALIZATION_ANNUAL_EXACT.R - EXACT MATCH TO ARCHIVED VISUALIZATION
################################################################################
# Visualization functions matching archived tributary map style EXACTLY
# Uses exact color coding and linewidths from archived visualization.R
################################################################################
...
#' Create tributary map with EXACT archived aesthetics (from archived visualization.R)
create_annual_tributary_map <- function(edges, basin, year, watershed, 
  ...
  # Use YlOrRd palette with 9 colors expanded to 10 (EXACT from archived visualization.R)
  pallete <- brewer.pal(9, "YlOrRd")
  pallete_expanded <- colorRampPalette(pallete)(10)
  
  # EXACT COLOR CODING from archived visualization.R lines 82-89
  ...
  # EXACT LINEWIDTHS from archived visualization.R
  stream_order_lwd <- edges$Str_Order
  linewidths <- rep(1, length(stream_order_lwd))
  
  if (watershed == "Yukon") {
    # YUKON LINEWIDTHS (from archived visualization.R lines 103-109)
    ...
    # KUSKOKWIM LINEWIDTHS (from archived visualization.R lines 111-119)
  ...
  # EXACT LEGEND from archived visualization.R lines 135-141
  legend("topleft", 
  ...
  # Add histogram overlay if provided (from archived visualization.R lines 145-164)
  if (!is.null(gg_hist)) {
    # Modify the histogram specifically for grid viewport use (from archived code)
    ...
    # Create viewport with explicit scaling (from archived code)
    vp_hist <- viewport(
      x = 0.5, y = 0.05, 
    ...
    # Print the modified histogram (from archived code)
...
cat("✓ EXACT MATCH Annual tributary mapping visualization functions loaded.\n")
cat("Functions match archived visualization.R exactly:\n")
cat("  - Same color bins for both watersheds: 0.0-0.2, 0.2-0.4, 0.4-0.6, 0.6-0.7, 0.7-0.8, 0.8-0.9, 0.9-1.0\n")
cat("  - Yukon: Conservative linewidths (max 3.7 for order 9)\n")
cat("  - Kusko: Dramatic linewidths (max 5.0 for order 9)\n")
cat("✓ Maps should now match archived versions EXACTLY!\n")