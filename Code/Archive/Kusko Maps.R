################################################################################
# KUSKOKWIM ANNUAL TRIBUTARY MAPS
# Standalone script to create annual production maps for Kuskokwim watershed
...
OUTPUT_DIR <- "/Users/benjaminmakhlouf/Research_repos/03_Shifting-Habitat-Mosaics-II/Maps/Kusko_Annual"
...
# Load spatial data
edges <- st_read(KUSKO_EDGES, quiet = TRUE)
basin <- st_read(KUSKO_BASIN, quiet = TRUE)
...
cat("Spatial data loaded. Processing", length(YEARS), "years...\n")
...
for (year in YEARS) {
  ...
  data_file <- file.path(DATA_DIR, paste0(year, "_Kusko_Natal_Origins_Genetics_CPUE.csv"))
  ...
  natal_data <- natal_data[!is.na(natal_data$natal_iso) & !is.na(natal_data$dailyCPUEprop), ]
  ...
  total_production <- sum(basin_assign_sum, na.rm = TRUE)
  cat("Total production:", round(total_production, 2), "\n")
  ...
  map_filename <- file.path(OUTPUT_DIR, paste0("Kusko_Annual_", year, ".png"))
  
  # Open PNG file
  png(file = map_filename, width = 9, height = 8, units = "in", res = 300, bg = "white")
  
  # Color coding (Kuskokwim style - 0.1 intervals)
  ...
  colcode[basin_assign_norm > 0 & basin_assign_norm <= 0.1] <- palette_colors[1]
  colcode[basin_assign_norm > 0.1 & basin_assign_norm <= 0.2] <- palette_colors[2]
  colcode[basin_assign_norm > 0.2 & basin_assign_norm <= 0.3] <- palette_colors[3]
  colcode[basin_assign_norm > 0.3 & basin_assign_norm <= 0.4] <- palette_colors[4]
  colcode[basin_assign_norm > 0.4 & basin_assign_norm <= 0.5] <- palette_colors[5]
  colcode[basin_assign_norm > 0.5 & basin_assign_norm <= 0.6] <- palette_colors[6]
  ...
  # Line widths (Kuskokwim style - dramatic differences)
  stream_order <- edges$Str_Order
  linewidths <- rep(1, length(stream_order))
  linewidths <- ifelse(stream_order == 9, 5, linewidths)
  linewidths <- ifelse(stream_order == 8, 4, linewidths)
  linewidths <- ifelse(stream_order == 7, 3, linewidths)
  linewidths <- ifelse(stream_order == 6, 2, linewidths)
  linewidths <- ifelse(stream_order == 5, 1.8, linewidths)
  linewidths <- ifelse(stream_order == 4, 1.5, linewidths)
  linewidths <- ifelse(stream_order == 3, 1, linewidths)
  
  # Plot
  par(mar = c(8, 4, 4, 2), bg = "white")
  plot(st_geometry(basin), col = 'gray60', border = 'gray60', 
       main = paste("Kuskokwim Annual Production", year), bg = "white")
  ...
  # Save CSV
  csv_filename <- file.path(OUTPUT_DIR, paste0("Kusko_Production_", year, ".csv"))
...
cat("Files created:\n")
for (year in YEARS) {
  map_file <- file.path(OUTPUT_DIR, paste0("Kusko_Annual_", year, ".png"))
  csv_file <- file.path(OUTPUT_DIR, paste0("Kusko_Production_", year, ".csv"))
