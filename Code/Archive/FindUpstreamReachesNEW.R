...
YukonNodes <- read.csv(here("/Users/benjaminmakhlouf/Research_repos/05_Shifting-Habitat-Mosaics-II/Data/UpstreamReaches/yukon_noderelationships.csv"),header=T,stringsAsFactors = F)#Node to reach relationships
...
kuskokwim_shapefile <- st_read('/Users/benjaminmakhlouf/Research_repos/05_Shifting-Habitat-Mosaics-II/SpatialData/Kusko_Reachbase.shp')
...
if(T){
  TribReachIDs <- FindUpstreamReachID_Kusk(12630)
  
  quartz()
  par(mfrow=c(1,1))
  nclr <- 9
  
  plotvar <- rep(0, nrow(kuskokwim_shapefile))
  plotvar[match(TribReachIDs, kuskokwim_shapefile$reachid)] <- 1
  colcode <- rep('gray', nrow(kuskokwim_shapefile))
  colcode[plotvar==0] <- 'gray60'
  colcode[plotvar==1] <- 'red'
  plot(st_geometry(kusk_basin), col = "gray", lwd=0.1)
  plot(st_geometry(kuskokwim_shapefile), col = colcode, pch=16, axes = F, add=TRUE, lwd=1)
}
