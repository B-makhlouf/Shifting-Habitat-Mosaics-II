...
p <- ggplot(combined_data, aes(x = DOY)) +
  # CPUE bars
  geom_col(aes(y = cpue), fill = "gray70", alpha = 0.8) +
  
  # Data availability markers
  geom_point(data = filter(combined_data, genetics_available), 
             aes(y = max(combined_data$cpue, na.rm = TRUE) * 1.05), 
             color = "#377eb8", shape = 15, size = 2, alpha = 0.7) +
  ...
  # Labels and theme
  labs(title = "Yukon River: CPUE and Data Availability by Year",
       subtitle = "Blue squares = Genetics available  |  Red circles = Otolith data available",
       x = "Day of Year",
...
# Save figure
output_file <- file.path(OUTPUT_DIR, "Yukon_Data_Availability_Overview.png")
