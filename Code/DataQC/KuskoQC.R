################################################################################
# KUSKOKWIM DATA AVAILABILITY FIGURE - CPUE + OTOLITHS
# (No genetics data - simple bar chart with otolith availability markers)
################################################################################

library(dplyr)
library(ggplot2)
library(readr)
library(tidyr)

# Configuration
DATA_DIR <- "/Users/benjaminmakhlouf/Research_repos/Schindler_GitHub/Arctic_Yukon_Kuskokwim_Data/Data/Natal Origin Analysis Data/03_Natal Origins Genetics CPUE"
OUTPUT_DIR <- "/Users/benjaminmakhlouf/Research_repos/05_Shifting-Habitat-Mosaics-II/Figures"
YEARS <- c(2017, 2018, 2019, 2020, 2021, 2022)

dir.create(OUTPUT_DIR, recursive = TRUE, showWarnings = FALSE)

# Compile data across all years
all_data <- list()

for (year in YEARS) {
  data_file <- file.path(DATA_DIR, paste0(year, "_Kusko_Natal_Origins_Genetics_CPUE.csv"))
  
  if (file.exists(data_file)) {
    df <- read_csv(data_file, show_col_types = FALSE) %>%
      mutate(
        year = year,
        has_otolith = !is.na(natal_iso)
      ) %>%
      group_by(year, DOY) %>%
      summarise(
        cpue = first(dailyCPUEprop),
        n_otolith = sum(has_otolith, na.rm = TRUE),
        n_total = n(),
        .groups = 'drop'
      ) %>%
      mutate(
        otolith_available = n_otolith > 0
      )
    
    all_data[[as.character(year)]] <- df
  }
}

# Combine all years
combined_data <- bind_rows(all_data)

# Calculate proportion of CPUE WITH otolith data for each year
data_coverage_summary <- combined_data %>%
  group_by(year) %>%
  summarise(
    total_cpue = sum(cpue, na.rm = TRUE),
    cpue_with_otolith = sum(cpue[otolith_available], na.rm = TRUE),
    prop_with_otolith = cpue_with_otolith / total_cpue,
    pct_with_otolith = round(prop_with_otolith * 100, 1),
    .groups = 'drop'
  ) %>%
  mutate(label = paste0(pct_with_otolith, "%"))

# Create the multi-panel figure
p <- ggplot(combined_data, aes(x = DOY)) +
  # CPUE bars - uniform color (no genetics to color by)
  geom_col(aes(y = cpue), fill = "#8DB1AB", alpha = 0.85) +
  
  # Data availability markers for otolith data
  geom_point(data = filter(combined_data, otolith_available), 
             aes(y = max(combined_data$cpue, na.rm = TRUE) * 1.1), 
             color = "#e41a1c", shape = 16, size = 2, alpha = 0.7) +
  
  # Facet by year
  facet_wrap(~year, ncol = 1, scales = "free_y") +
  
  # Add text showing % CPUE with otolith data
  geom_text(data = data_coverage_summary, 
            aes(x = Inf, y = Inf, label = label),
            hjust = 1.1, vjust = 1.5, size = 4, 
            color = "firebrick", fontface = "bold") +
  
  # Labels and theme
  labs(title = "Kuskokwim River: CPUE and Data Availability by Year",
       subtitle = "Red circles = Otolith data (natal isotope) available",
       x = "Day of Year",
       y = "Daily CPUE Proportion") +
  theme_minimal() +
  theme(
    strip.text = element_text(size = 12, face = "bold"),
    plot.title = element_text(size = 14, face = "bold"),
    plot.subtitle = element_text(size = 10, color = "gray40"),
    panel.grid.minor = element_blank(),
    panel.spacing = unit(1, "lines")
  )

# Save figure
output_file <- file.path(OUTPUT_DIR, "Kusko_Data_Availability_Overview.png")
ggsave(output_file, p, width = 12, height = 10, dpi = 300)

cat("\n✓ Saved:", output_file, "\n")

# Print summary table
cat("\n=== DATA AVAILABILITY SUMMARY ===\n")
summary_table <- combined_data %>%
  group_by(year) %>%
  summarise(
    days_total = n(),
    days_with_otolith = sum(otolith_available),
    .groups = 'drop'
  )

print(summary_table)

cat("\n=== CPUE WITH OTOLITH DATA ===\n")
print(data_coverage_summary %>% select(year, total_cpue, cpue_with_otolith, pct_with_otolith))