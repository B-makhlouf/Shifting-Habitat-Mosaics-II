...
Kusko_map <- function(year, sensitivity_threshold, quartile = "ALL",min_error) {   
  ...
  pid_isose_mod <- ifelse(pid_isose < min_error, min_error, pid_isose) # Bumps super low error areas up
...
Map_Base_Kusko <- function(River, plotvar, identifier, sensitivity_threshold) {
  ...
  colcode <- rep("gray60", length(plotvar)) # Default color for edges
