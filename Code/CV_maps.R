...
generate_cv_plot <- function(threshold) {
  ...
  # Add a value of 0 to CV for the bottom 70% of means
  CV_data$cv[CV_data$mean < median(CV_data$mean)] <- 0
...
generate_change_plot <- function(threshold) {
  # Read in production data with CV metrics
  CV_data <- read_csv(here("Results/Variability", paste0("Yukon_Production_all_", threshold, ".csv")))
  
  # Extract the variation vector and handle NAs
  change <- CV_data$change_2016_2017
  change[is.na(change)] <- 0
  
  # Calculate breaks
  breaks <- seq(min(change), max(change), length = 9)
  if (max(change) > max(change)) {
    breaks <- seq(min(change[change > 0]), max(change), length = 10)
  }
  ...
  # Filepath for saving the plot
  filename <- paste0("change_16_17", threshold, "_.pdf")
  filepath <- file.path(here("Figures", "Maps", "Yukon", filename))
...
for (threshold in thresholds) {
  generate_change_plot(threshold)
}
