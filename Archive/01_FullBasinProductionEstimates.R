################################################################################
# FULL BASIN FULL YEAR PRODUCTION MAPS
#
# Two functions drive all annual production analyses:
#   run_kusko(year)  -> Kuskokwim
#   run_yukon(year)  -> Full Yukon basin (Lower + Middle + Upper)
#
# Outputs per call:
...
PATHS <- list(
  ...
  map_kusko      = here("Figures", "Maps","FullBasin", "Kusko"),
  map_yukon_full = here("Figures", "Maps","FullBasin", "Yukon")
)
...
run_kusko <- function(year) {
  ...
  fixed_prior        <- stream_order_prior * edges$UniPh2oNoE  * presence_prior #*new_habitat_prior
