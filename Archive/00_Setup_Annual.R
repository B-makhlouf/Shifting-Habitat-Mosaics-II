################################################################################
# 00_SETUP_ANNUAL.R - EXACT MATCH TO ORIGINAL ARCHIVED CODE
################################################################################
...
# CORRECTED: Now uses EXACT parameters from original archived main.R
################################################################################
...
CONFIG <- list(
  ...
  yukon_years = c(2015, 2016, 2017, 2018, 2021),  # Yukon has different available years
...
# EXACT ORIGINAL: Watershed-specific parameters from archived main.R lines 61-66
WATERSHED_PARAMS <- list(
  ...
  Yukon = list(
    min_stream_order = 4,        # From archived main.R line 66
    min_error = 0.003,           # From archived main.R line 65
    sensitivity_threshold = 0.8  # From archived main.R line 64
  )
)
...
perform_assignment <- function(natal_data, edges, watershed, priors, pid_iso, error, sensitivity_threshold) {
  ...
  for (i in 1:n_fish) {
    ...
    if (watershed == "Kusko") {
      # KUSKOKWIM ASSIGNMENT: Simple isotope + spatial priors (NO management filtering)
...
cat("✓ Annual tributary mapping setup complete - EXACT MATCH TO ORIGINAL.\n")
cat("✓ Uses EXACT parameters from archived main.R:\n")
cat("  - Yukon: min_stream_order=4, sensitivity_threshold=0.8\n")
cat("  - Kusko: min_stream_order=3, sensitivity_threshold=0.7\n")
cat("✓ Available years:\n")
cat("  - Kusko:", paste(CONFIG$kusko_years, collapse = ", "), "\n")
cat("  - Yukon:", paste(CONFIG$yukon_years, collapse = ", "), "\n")
cat("✓ Next: source('01_visualization_annual_EXACT.R') and '02_annual_tributary_maps.R'\n")